...
def get_subscription_id():
    ...
    """
    result = subprocess.run(['az', 'account', 'subscription', 'list'],
...
def incident_show(incident_id, resource_group, workspace_name):
    ...
    """
    result = subprocess.run(['az', 'sentinel', 'incident', 'show',
...
def run_playbook(subscription_id, incident_id, resource_group, workspace_name,
                 playbook_name):
    """
    ...
    success.
    """
    logic_apps_resource_id = ("/subscriptions/" + subscription_id
                              + "/resourceGroups/" + resource_group
                              + "/providers/Microsoft.Logic/workflows/"
                              + playbook_name)
    result = subprocess.run(['az', 'sentinel', 'incident', 'run-playbook',
                             '--incident-identifier', incident_id,
                             '--resource-group', resource_group,
                             '--workspace-name', workspace_name,
                             '--logic-apps-resource-id', logic_apps_resource_id],
                            stdout=subprocess.PIPE)
...
def get_one_incident_id(resource_group, workspace_name):
    ...
    """
    result = subprocess.run(['az', 'sentinel', 'incident', 'list',
                             '--resource-group', resource_group,
                             '--workspace-name', workspace_name],
                            stdout=subprocess.PIPE)
